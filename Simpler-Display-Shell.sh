...
if [ ${#menu_options[@]} -eq 0 ]; then
    whiptail --title "Error" --msgbox "No active video outputs detected." 6 50
    exit 1
fi
...
while true; do
    case "$STATE" in
        "SELECT_OUTPUT")
            whiptail --title "Detected Active Outputs" \
            ...
            if [ $RET -eq 255 ]; then
                clear
                echo "EXITED: Escaped. Session terminated."
                exit 0
            elif [ $RET -ne 0 ]; then
                clear
        ...
        "SELECT_PARAMETER")
            ...
            whiptail --title "Interface $DISPLAY_NAME: Identified Parameters" \
        ...
        "RES_GEOMETRY")
            ...
            whiptail --title "Interface $DISPLAY_NAME - Identified Resolutions" \
        ...
        "RES_REFRESH")
            ...
            whiptail --title "Interface $DISPLAY_NAME - Identified Refresh Rates" \
        ...
        "SCALE_SELECT")
            ...
            whiptail --title "Interface $DISPLAY_NAME - Display Scaling (Currently: $CURRENT_SCALE)" \
            ...
            if [ "$SCALE_CHOICE" -eq "$CUSTOM_IDX" ]; then
                whiptail --title "Custom Display Scale Input" \
                ...
                if [[ ! "$SELECTED_SCALE" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
                    whiptail --title "Error" --msgbox "Invalid input. Not a number." 6 60
                    STATE="SCALE_SELECT"
                    continue
                fi
                ...
                if [ "$VALID" -ne 1 ]; then
                    whiptail --title "Error" --msgbox "Numeber is not between 0.5 and 3 (0.5<[New Value]<3)." 6 60
                    STATE="SCALE_SELECT"
                    continue
                fi
            else
        ...
        "HDR_SELECT")
            ...
            whiptail --title "Interface $DISPLAY_NAME - High Dynamic Range (HDR)" \
        ...
        "VRR_SELECT")
            ...
            whiptail --title "Interface $DISPLAY_NAME - Variable Refresh-Rate (VRR)" \
        ...
        "EXECUTE")
            ...
            whiptail --title "Waiting To Confirm Changes" --defaultno \
                --yesno "Changes Applied:\nInterface: $DISPLAY_NAME\n$HUMAN_MSG\n\nConfirm changes?" 10 60
